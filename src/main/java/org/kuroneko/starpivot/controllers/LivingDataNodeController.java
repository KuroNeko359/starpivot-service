...
/**
 * REST 控制器，用于管理存活的 Hadoop DataNode 节点信息。
 * 该控制器提供 HTTP 接口，用于查询当前活跃的 DataNode 节点的状态数据。
 * 所有请求以 "/datanodes" 作为基础路径，通过 Spring MVC 框架处理。
 */
@RestController
...
public class LivingDataNodeController {
    ...
    /**
     * DataNode 服务实例，通过 Spring 的依赖注入自动装配。
     * 该服务负责与底层 Hadoop 集群交互，获取 DataNode 相关数据。
     */
    @Autowired
    private DataNodeService dataNodeInfo;

    /**
     * 获取当前存活的 DataNode 节点信息。
     * 该方法通过 HTTP GET 请求返回一个映射表，键为节点标识符，值为对应的节点信息对象。
     * 请求路径为 "/datanodes/alive"。
     *
     * @return 一个 Map 对象，其中键为节点标识符 (String)，值为节点信息对象
     *         ({@link DataNodeInfo.NodeInfo})。如果没有存活节点，则返回空 Map。
     * @see DataNodeService#getAliveDataNodes()
     */
    @GetMapping("/alive")
